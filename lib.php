 ...
 * of the new instance.
 *
 * @param stdClass $throwquestions Submitted data from the form in mod_form.php
 * @param mod_throwquestions_mod_form $mform The form instance itself (if needed)
...
function throwquestions_add_instance(stdClass $throwquestions, mod_throwquestions_mod_form $mform = null) {
    ...
    $throwquestions->id = $DB->insert_record('throwquestions', $throwquestions);

    throwquestions_grade_item_update($throwquestions);

    return $throwquestions->id;
}
 ...
 *
 * @param stdClass $throwquestions An object from the form in mod_form.php
 * @param mod_throwquestions_mod_form $mform The form instance itself (if needed)
...
function throwquestions_delete_instance($id) {
    ...
    if (! $throwquestions = $DB->get_record('throwquestions', array('id' => $id))) {
        return false;
    }

    // Delete any dependent records here.

    $DB->delete_records('throwquestions', array('id' => $throwquestions->id));
 ...
 * @param stdClass $throwquestions The throwquestions instance record
 ...
 * @param stdClass $throwquestions the module instance record
 */
 ...
 *
 * @param int $throwquestionsid ID of an instance of this module
...
function throwquestions_scale_used($throwquestionsid, $scaleid) {
    ...
    if ($scaleid and $DB->record_exists('throwquestions', array('id' => $throwquestionsid, 'grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
function throwquestions_scale_used_anywhere($scaleid) {
    ...
    if ($scaleid and $DB->record_exists('throwquestions', array('grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
 ...
 *
 * @param stdClass $throwquestions instance object with extra cmidnumber and modname property
...
function throwquestions_grade_item_update(stdClass $throwquestions, $reset=false) {
    ...
    grade_update('mod/throwquestions', $throwquestions->course, 'mod', 'throwquestions',
            $throwquestions->id, 0, null, $item);
}

/**
 * Delete grade item for given throwquestions instance
 *
 * @param stdClass $throwquestions instance object
 * @return grade_item
 */
function throwquestions_grade_item_delete($throwquestions) {
    ...
    return grade_update('mod/throwquestions', $throwquestions->course, 'mod', 'throwquestions',
            $throwquestions->id, 0, null, array('deleted' => 1));
}
 ...
 * @param stdClass $context the throwquestions's context
 ...
 *
 * @param navigation_node $navref An object representing the navigation tree node of the throwquestions module instance
 * @param stdClass $course current course record
 * @param stdClass $module current throwquestions instance record
 ...
 * @param navigation_node $throwquestionsnode throwquestions administration node
 */
